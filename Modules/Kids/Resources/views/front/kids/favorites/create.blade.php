...
<head>
    ...
    <style>
        ...
        .styled-input {
            background-color: #F8FCFC; /* Replace with your desired background color */
            border: 1px solid #F8FCFC; /* Add a default border */
            width: 95%; /* Add a default border */
        }

        .styled-input:focus {
            outline: none; /* Remove the default focus outline */
            border-color: none; /* Remove the border on focus */
        }
...
<nav aria-label="breadcrumb mt-5 mb-5">
    <div class="container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">
                <a href="{{route('kids.evaluate.favorites.index',$kid->id)}}"><i class="fa-solid fa-chevron-left"></i>
                    تقيم المفضلات
                </a>
            </li>
...
<script>
    function displayFavouriteDetails(name, countOfChoices, secondItem, percentageFirstTable, fourthItem, fifthItem) {
        ...
        $(fourthItem).text(firstItemClickCount + ' مرات');
        $(fifthItem).text(displayEquation);
    }
        ...
        if (inputs.every(input => $(input).val() !== '')) {
            ...
            if ($modalActiveElement.length > 0) {
                if ($modalActiveElement.hasClass("firstItem")) {
                    ...
                    displayFavouriteDetails($('.firstFavoriteName'), '.c-rating-favorite .firstItem.modalActive', '.firstFavoriteName',
                        '.firstFavoritePercentage', '#firstFavoriteCount', '#firstFavoritePercentage'
                    );
                } else if ($modalActiveElement.hasClass("secondItem")) {

                    displayFavouriteDetails($('.secondFavoriteName'), '.c-rating-favorite .secondItem.modalActive', '.secondFavoriteName',
                        '.secondFavoritePercentage', '#secondFavoriteCount', '#secondFavoritePercentage'
                    );
                } else if ($modalActiveElement.hasClass("thirdItem")) {

                    displayFavouriteDetails($('.thirdFavoriteName'), '.c-rating-favorite .thirdItem.modalActive', '.thirdFavoriteName',
                        '.thirdFavoritePercentage', '#thirdFavoriteCount', '#thirdFavoritePercentage'
                    );
                } else if ($modalActiveElement.hasClass("fourthItem")) {

                    displayFavouriteDetails($('.fourthFavoriteName'), '.c-rating-favorite .fourthItem.modalActive', '.fourthFavoriteName',
                        '.fourthFavoritePercentage', '#fourthFavoriteCount', '#fourthFavoritePercentage'
                    );
                } else if ($modalActiveElement.hasClass("fifthItem")) {

                    displayFavouriteDetails($('.fifthFavoriteName'), '.c-rating-favorite .fifthItem.modalActive', '.fifthFavoriteName',
                        '.fifthFavoritePercentage', '#fifthFavoriteCount', '#fifthFavoritePercentage'
                    );
                } else if ($modalActiveElement.hasClass("sixthItem")) {

                    displayFavouriteDetails($('.sixthFavoriteName'), '.c-rating-favorite .sixthItem.modalActive', '.sixthFavoriteName',
            ...
            if ($(this).hasClass('firstItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .firstItem.modalActive', '.firstFavoriteName',
                    '.firstFavoritePercentage', '#firstFavoriteCount', '#firstFavoritePercentage'
                );
            } else if ($(this).hasClass('secondItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .secondItem.modalActive', '.secondFavoriteName',
                    '.secondFavoritePercentage', '#secondFavoriteCount', '#secondFavoritePercentage'
                );
            } else if ($(this).hasClass('thirdItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .thirdItem.modalActive', '.thirdFavoriteName',
                    '.thirdFavoritePercentage', '#thirdFavoriteCount', '#thirdFavoritePercentage'
                );
            } else if ($(this).hasClass('fourthItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .fourthItem.modalActive', '.fourthFavoriteName',
                    '.fourthFavoritePercentage', '#fourthFavoriteCount', '#fourthFavoritePercentage'
                );
            } else if ($(this).hasClass('fifthItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .fifthItem.modalActive', '.fifthFavoriteName',
                    '.fifthFavoritePercentage', '#fifthFavoriteCount', '#fifthFavoritePercentage'
                );
            } else if ($(this).hasClass('sixthItem')) {
                displayFavouriteDetails($(this), '.c-rating-favorite .sixthItem.modalActive', '.sixthFavoriteName',
                    '.sixthFavoritePercentage', '#sixthFavoriteCount', '#sixthFavoritePercentage'
                );
            } else {
                console.log('null');
