        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-6 d-flex align-items-center justify-content-md-end justify-content-start ">
                <button class="add-session border-0 edit-file mt-md-4 mb-md-4 mb-3 mt-2">
                    <a class="add-new-appeals"> <img src="{{asset('dist/front/assets/images/plus.png')}}" alt=""> إضافة
                        تقييم
                        جديد </a>
                </button>
                <button type="submit" class="border-0 edit-file mt-md-4 mb-md-4 mb-3 mt-2 me-3">
                    <a href="{{ route('kids.evaluate.appeals.vertical-draw', $kid->id) }}"> <img
                            src="{{asset('dist/front/assets/images/paint.png')}}" alt=""> الرسم العمودي للتقييم </a>
                </button>
            </div>
                        ...
                        <div class="dates row" style="border-radius: 8px;overflow: hidden;">
                            @foreach ($sessions as $key =>  $val)
                                <div class="date col-lg-2 col-md-3 col-6 text-center"
                                    ...
                                    @if ($loop->last)
                                        <div class="close" data-id="{{$val->session->id}}" style="top:0 !important;">
                                    ...
                                    <h9 class="sessions-date " data-id="{{$val->session->id}}"
                                        ...
                                        <span> تقييم بتاريخ</span> {{date('Y/m/d', strtotime($val->created_at))}} </h9>
                                </div>
                            @endforeach
                        </div>
                                ...
                                @foreach ($userSessions->Appsessions as $key => $val)
                                    <li class="nav-item">
                                        <a class="nav-link {{$val->Appale->name == 'A' ? 'active' : ''}}"
                                           data-bs-toggle="tab"
                                           href="#{{$val->Appale->name}}">{{$val->Appale->name}}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                            ...
                            @foreach ($userSessions->Appsessions as $key => $val)
                                ...
                                <div id="{{$val->Appale->name}}"
                                                        ...
                                                        @foreach ($num->Appale_Nums->Appale_Ques as $ke => $quest)
                                                            ...
                                                            @if ($num->ans_old_id != null)
                                                                @if ($num->ans_old_id >= $quest->id)
                                                                    <span class="stars"
                                                                          style="display: {{$num->ans_old_id > $quest->id ? 'none' : 'block'}} !important;background-color: {{$num->hex_old}};"
                                                                          data-quest="{{$num->Appale_Nums->id}}"
                                                                          data-id="{{$quest->id}}">{{$ke + 1}}</span>

                                                                @else
                                                                    <span class="star"
                                                                          style="display: {{$num->ans_id > $quest->id ? 'none' : 'block'}} !important;background-color: {{$num->ans_id >= $quest->id ? $userSessions->Session->hex : ''}};"
                                                                          data-quest="{{$num->Appale_Nums->id}}"
                                                                          data-id="{{$quest->id}}">{{$ke + 1}}</span>
                                    ...
                                    <div class="" style="position:relative;">
                                        ...
                                        <textarea id="placeholder-teaxtarea" name="desc[{{$val->Appale->id}}]"
                                                  class="textarea"> {{$val->desc}} </textarea>
                                    </div>
        ...
        $.ajax({
            ...
            success: function () {
                ...
                closeButtons.forEach(closeButton => {
                    ...
                    window.location.href = "{{route('kids.evaluate.appeals.create',$kid->id)}}"
                });
            }
        });
    });
